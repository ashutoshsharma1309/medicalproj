...
work. One place now owns the sequence every reading goes through, in order:
    ...
    authenticate device → validate → normalise → persist → evaluate → publish
...
can check; publishing comes last because a dashboard is the one consumer that
can afford to miss a message.
...
class SensorProcessingService:
    ...
    async def process(self, token: str | None, body: object) -> ProcessedReading:
        ...
        return ProcessedReading(
            reading_id=reading_id, device=device, reading=reading, alerts=alerts
        )
